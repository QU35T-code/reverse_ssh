...
func Run(addr, fingerprint, proxyAddr string) {
	...
	config := &ssh.ClientConfig{
		...
		HostKeyCallback: func(hostname string, remote net.Addr, key ssh.PublicKey) error {
			...
			if sources.FingerprintSHA256Hex(key) != fingerprint {
				return fmt.Errorf("Server public key invalid, expected: %s, got: %s", fingerprint, sources.FingerprintSHA256Hex(key))
			}

			return nil
		},
		ClientVersion: "SSH-" + sources.Version,
	}
	...
	for { // My take on a golang do {} while loop :P
		...
		user, err := sources.CreateUser(sshConn)
		if err != nil {
			clientLog.Warning("Unable to create sources user")
			return
		}
