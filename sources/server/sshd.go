...
func acceptConn(c net.Conn, config *ssh.ServerConfig, timeout int, dataDir string) {
	...
	realConn := &sources.TimeoutConn{c, time.Duration(timeout) * time.Minute}
	...
	case "client":
		...
		go func() {
			...
			err = sources.RegisterChannelCallbacks(nil, chans, clientLog, map[string]sources.ChannelHandler{
