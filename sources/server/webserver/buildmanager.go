...
func Build(goos, goarch, suppliedConnectBackAdress, fingerprint, name string, shared, upx, garble bool) (string, error) {
	...
	filename, err := sources.RandomString(16)
	if err != nil {
		return "", err
	}

	if len(name) == 0 {
		name, err = sources.RandomString(16)
		if err != nil {
			return "", err
		}
	}
	...
	f.Version = sources.Version + " (guess)"
	...
	newPrivateKey, err := sources.GeneratePrivateKey()
	if err != nil {
		return "", err
	}
	...
	err = os.WriteFile(filepath.Join(projectRoot, "sources/client/keys/private_key"), newPrivateKey, 0600)
	if err != nil {
		return "", err
	}
	...
	err = os.WriteFile(filepath.Join(projectRoot, "sources/client/keys/private_key.pub"), publicKeyBytes, 0600)
	if err != nil {
		return "", err
	}

	buildArguments = append(buildArguments, fmt.Sprintf("-ldflags=-s -w -X main.destination=%s -X main.fingerprint=%s -X github.com/QU35T-code/reverse_ssh/sources.Version=%s", suppliedConnectBackAdress, fingerprint, f.Version))
