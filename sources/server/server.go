...
func CreateOrLoadServerKeys(privateKeyPath string) (ssh.Signer, error) {
	...
	if _, err := os.Stat(privateKeyPath); os.IsNotExist(err) {
		...
		privateKeyPem, err := sources.GeneratePrivateKey()
		if err != nil {
...
func Run(addr, dataDir, connectBackAddress string, insecure, enabledWebserver bool, timeout int) {
	...
	log.Println("Version: ", sources.Version)
	var err error
	...
	log.Println("Server key fingerprint: ", sources.FingerprintSHA256Hex(private.PublicKey()))
