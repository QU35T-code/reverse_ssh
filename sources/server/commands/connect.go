...
func createSession(sshConn ssh.Conn, ptyReq sources.PtyReq, shell string) (sc ssh.Channel, err error) {
	...
	_, err = splice.SendRequest("shell", true, ssh.Marshal(sources.ShellStruct{Cmd: shell}))
	if err != nil {
...
RequestsProxyPasser:
	for {
		...
		case r := <-currentClientRequests:
			response, err := sources.SendRequest(*r, newSession)
			if err != nil {
